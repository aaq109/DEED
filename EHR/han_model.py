...
class HierarchialAttentionNetwork(nn.Module):
    ...
    def __init__(self, n_classes, vocab_size, emb_size, word_rnn_size, sentence_rnn_size, word_rnn_layers,
                 sentence_rnn_layers, word_att_size, sentence_att_size, edl, hdf_len, dropout=0.5):
        """
        ...
        # Classifier
        if edl:
            self.fc = nn.Linear(2 * sentence_rnn_size, n_classes*2)
        else:
    ...
    def forward(self, X):
        documents, sentences_per_document, words_per_sentence, hdf = X[0], X[1], X[2], X[3]
        """
        Forward propagation.
        ...
        return scores, word_alphas, sentence_alphas, document_embeddings
...
class SentenceAttention(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size, word_rnn_size, sentence_rnn_size, word_rnn_layers, sentence_rnn_layers,
        ...
        self.sentence_rnn = nn.GRU(2 * word_rnn_size + hdf_len, sentence_rnn_size, num_layers=sentence_rnn_layers,
    ...
    def forward(self, documents, sentences_per_document, words_per_sentence, hdf):
        """
        Forward propagation.
        ...
        sentences = torch.cat((sentences, packed_hdf.data), dim=1)
